...
export function Overview({ profile, stats, publicStats, authHeadersExist, changeView, places, getImgUrl, setSelectedPlace, onShare }) {
    return (
...
export function MyVenuesView({ profile, places, editingPlaceId, setEditingPlaceId, placeForm, setPlaceForm, onSave, onStartEdit, onDelete, onSelect, setRejectModal, setRejectReason, updatePlaceStatus, run, initialPlace, getImgUrl, search, searchCategory, forcePersonalOnly, forceGlobalOnly, onShare }) {
    const isAdmin = profile?.role === "admin";
        ...
        <div className="animate-fade-in">
                ...
                <>
                    ...
                    <div className="grid">
                        {filteredPlaces.filter(p => p.owner?._id === profile?._id).map(p => (
                            <VenueCard key={p._id} p={p} profile={profile} getImgUrl={getImgUrl} onEdit={() => onStartEdit(p)} onDelete={() => onDelete(p._id)} onSelect={() => onSelect(p)} onShare={onShare} />
                        ))}
            ...
            {showGlobal && (
                <>
                    ...
                    <div className="grid">
                        {filteredPlaces.filter(p => p.owner?._id !== profile?._id).map(p => (
                            <VenueCard key={p._id} p={p} profile={profile} getImgUrl={getImgUrl} onEdit={null} onDelete={() => onDelete(p._id)} onSelect={() => onSelect(p)} onShare={onShare} />
                        ))}
                        {filteredPlaces.filter(p => p.owner?._id !== profile?._id).length === 0 && (
